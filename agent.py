...
graphiti = Graphiti(NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD, llm_client=gemini_client)
...
class CareerAgent:
    ...
    def _handle_holland_phase(self, user_answer):
        ...
        if next_q_index < len(HOLLAND_QUESTIONS):
            ...
            return self._transition_to_gemini_phase()

    def _transition_to_gemini_phase(self):
    ...
    async def _handle_gemini_phase(self, user_answer):
        ...
        # This is a simplified filtering logic for the demo.
        # A more robust version would use Graphiti search results to run a complex Cypher query.
        ...
        if len(filtered_careers) <= 5:
            ...
            prompt = f"Remaining options: {filtered_careers[:10]}... Ask the next logical question to narrow these down."
            response = gemini_model.generate_content(prompt)
